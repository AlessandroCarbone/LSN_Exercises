...
void Input(void){       //Prepare all stuff for the simulation
  ...
  //Random Numbers Generator
    ...
    //Bin size for the evaluation of g(r)
    ...
    if(read==false){    //Just actual configuration
        ...
        sumv2 /= (double)npart;
        cout<<"Square velocity for particle = "<<sumv2<<endl;
...
void Measure(void){     //Properties measurement
    ...
    if(phase==1){
        ...
        Gofr.open("gas/output_gofr.gas",ios::app);
    }
    if(phase==2){
        ...
        Gofr.open("liquid/output_gofr.liquid",ios::app);
    }
    if(phase==3){
        ...
        Gofr.open("solid/output_gofr.solid",ios::app);
    }
    ...
    stima_press = rho*stima_temp + w/(double)(3*vol) + ptail*rho;  //Pressure
    ...
    //The normalization of the histogram is inside the function Average
...
void Average(int nblocks){
    ...
    vector <double> vg(nbins);
    vector <double> vg2(nbins);
    double V=0;
    ...
    if(phase==1){
        ...
        input_g.open("gas/output_gofr.gas");
        ...
        output_g.open("gas/ave_gofr.gas");
    }
    ...
    if(phase==2){
        ...
        input_g.open("liquid/output_gofr.liquid");
        ...
        output_g.open("liquid/ave_gofr.liquid");
    }
    if(phase==3){
        ...
        input_g.open("solid/output_gofr.solid");
        ...
        output_g.open("solid/ave_gofr.solid");
    }
    ...
    //Di seguito si memorizzano le varie misure di g(r=j*bin_size)
    for(int i=0;i<nstep;i++){
        for(int j=0;j<nbins;j++){
            V=rho*npart*(4./3.)*M_PI*(pow((j+1)*bin_size,3)-pow(j*bin_size,3));     //Normalization
            input_g>>val;
            val=val/V;
            if(phase==1){
                if(i>=(nstep-L*50)){     //Si considerano solo gli ultimi 50 blocchi per tenere conto dell'equilibrazione
                    vg[j]+=val;
                    vg2[j]+=val*val;
                }
            }
    ...
    if(phase==1){
        for(int k=0;k<nbins;k++){
            output_g<<k*bin_size<<" "<<vg[k]<<" "<<(vg2[k]-pow(vg[k],2))/(double)(L*50.)<<endl;
        }
    }
    else{
        for(int k=0;k<nbins;k++){
            output_g<<k*bin_size<<" "<<vg[k]<<" "<<(vg2[k]-pow(vg[k],2))/nstep<<endl;
        }
    }
